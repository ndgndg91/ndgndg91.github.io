    ...
    <div class="mt-6 prose dark:prose-invert prose-code:bg-gray-100 prose-code:dark:bg-gray-800 prose-code:px-1 prose-code:rounded prose-code:text-gray-900 prose-code:dark:text-gray-100 prose-headings:text-gray-900 prose-headings:dark:text-white prose-p:text-gray-700 prose-p:dark:text-gray-400 prose-strong:text-gray-900 prose-strong:dark:text-white">
      <div class="mt-6">
        ...
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          <li>Simplest configuration approach</li>
          <li>Ensures uniform load distribution across consumers</li>
          <li>Each pod handles exactly one partition</li>
          <li>Scales linearly with partition count</li>
        </ul>
        <h3 class="inline-block mb-2 text-xl tracking-tight text-gray-900 dark:text-white mt-4">Disadvantages:</h3>
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          <li>Increased overhead for partition leader election and metadata management</li>
          <li>Higher resource consumption across cluster</li>
          <li>More complex rebalancing when pods join/leave the cluster</li>
          <li>Limited by maximum partition count of a topic</li>
        </ul>
      </div>
      ...
      <div class="mt-6">
        ...
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          <li>Maximum concurrency is limited by the number of topic partitions</li>
          <li>Threads exceeding the number of partitions will stay idle</li>
          <li>High concurrency may cause increased CPU and memory pressure</li>
          <li>The default concurrency value is 1 (single-threaded message consumption)</li>
          <li>Concurrency should generally not exceed the number of partitions assigned to the consumer instance</li>
        </ul>
        ...
        <div class="relative group">
          ...
          <pre class="bg-gray-900 text-gray-100 p-4 rounded-lg mb-6 overflow-x-auto"><code class="language-kotlin"><span class="text-[#808080]">@Bean</span>
<span class="text-[#569CD6]">fun</span> <span class="text-[#DCDCAA">kafkaListenerContainerFactory</span>(
    ...
    <span class="text-[#808080]">// Other code omitted</span>
      ...
      <div class="mt-6">
        ...
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          <li>Allows multiple threads to process messages from a single partition</li>
          <li>Bypasses scaling limits imposed by partition count</li>
          <li>Overcomes batch listener limitations, offering better error handling and retry capabilities</li>
          <li>Supports fine-grained per-message acknowledgments</li>
        </ul>

        <h3 class="inline-block mb-2 text-xl tracking-tight text-gray-900 dark:text-white mt-4">Limitations:</h3>
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          <li>Limited community references and documentation</li>
          <li>No direct integration with Spring Kafka (as of April 2025)</li>
          <li>Independent from the Spring ecosystem</li>
        </ul>

        <h3 class="inline-block mb-2 text-xl tracking-tight text-gray-900 dark:text-white mt-4">Resources:</h3>
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          <li><a href="https://github.com/confluentinc/parallel-consumer" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">GitHub - confluentinc/parallel-consumer</a></li>
          <li><a href="https://www.confluent.io/blog/introducing-confluent-parallel-consumer/" class="text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300">Introducing Confluent's Parallel Consumer Message Processing Client</a></li>
        </ul>
      </div>
...
<span class="text-[#569CD6]">class</span> <span class="text-[#4EC9B0]">KafkaMessageHandler</span>(
    ...
    <span class="text-[#569CD6]">private fun</span> <span class="text-[#DCDCAA">processMessage</span>(<span class="text-[#9CDCFE">message</span>: <span class="text-[#4EC9B0]">ConsumerRecord</span><<span class="text-[#4EC9B0]">String</span>, <span class="text-[#4EC9B0]">String</span>>) {
        ...
        <div class="relative group">
          ...
          <pre class="bg-gray-900 text-gray-100 p-4 rounded-lg mb-6 overflow-x-auto"><code class="language-kotlin"><span class="text-[#808080]">@Bean</span>(destroyMethod = <span class="text-[#CE9178]">"shutdown"</span>)
<span class="text-[#569CD6]">fun</span> <span class="text-[#DCDCAA">asyncExecutor</span>(): <span class="text-[#4EC9B0]">ExecutorService</span> {
  <span class="text-[#569CD6]">return</span> <span class="text-[#4EC9B0]">Executors</span>.<span class="text-[#DCDCAA">newFixedThreadPool</span>(<span class="text-[#B5CEA8">10</span>)
        ...
        <ul class="w-full space-y-1 text-gray-700 list-disc list-inside dark:text-gray-400">
          ...
          <li>Consumer thread quickly acknowledges messages and continues</li>
          <li>Good for operations where reliability isn't critical</li>
        </ul>
      ...
      <div class="mt-6">
        ...
        <div class="relative group">
          ...
          <pre class="bg-gray-900 text-gray-100 p-4 rounded-lg mb-6 overflow-x-auto"><code class="language-java"><span class="text-[#808080]">@Bean</span>
